...
import type { TeamEndData } from "@/entities/user";
import { useGetTeamDetail } from "@/entities/user";
import { InvestCompleteModal } from "@/features/stock-trading";
import { roundToFixed, useLocalStorage, useUnchangedValue } from "@/shared";
...
const mockTradeData: TeamEndData[] = [
  {
    itemId: 101,
    itemName: "모의주식A",
    orderType: "BUY",
...
export const HistorySidebar = ({ isMock = false }: { isMock?: boolean }) => {
  ...
  const [realTradeData, setRealTradeData] = useLocalStorage<TeamEndData[]>("trade", []);
  ...
  const setCashMoneyStable = useCallback(
    (value: number) => {
  ...
  const totalBuy = tradeData
    .filter((tradeItem: TeamEndData) => tradeItem.orderType === "BUY")
    .reduce((acc: number, item: TeamEndData) => acc + item.totalMoney, 0);
  const totalSell = tradeData
    .filter((tradeItem: TeamEndData) => tradeItem.orderType === "SELL")
    .reduce((acc: number, item: TeamEndData) => acc + item.totalMoney, 0);
  ...
  const handleDeleteTransaction = (deletedId: number) => {
    ...
    const deletedTrade = realTradeData.find((tradeItem: TeamEndData) => tradeItem.itemId === deletedId);
    ...
    setRealTradeData(realTradeData.filter((tradeItem: TeamEndData) => tradeItem.itemId !== deletedId));
  };

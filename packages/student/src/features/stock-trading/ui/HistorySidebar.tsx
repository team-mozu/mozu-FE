...
import { type TeamEndProps, useGetTeamDetail } from "@/entities/user";
...
export const HistorySidebar = ({ isMock = false }: { isMock?: boolean }) => {
  ...
  const tradeData = isMock ? mockTradeData : allTradeData.filter(trade => trade.invCount === currentInvRound);
  ...
  useEffect(() => {
    if (!isMock && realTeamData?.cashMoney !== undefined && realTeamData?.curInvRound !== undefined) {
      ...
      if (lastSyncedInvRound === null || lastSyncedInvRound !== currentInvRound) {
        ...
        if (lastSyncedInvRound !== null && lastSyncedInvRound !== currentInvRound) {
          console.log(
            `[HistorySidebar] 투자 라운드 변경 - 현재 라운드 거래만 표시 (${lastSyncedInvRound} → ${currentInvRound})`,
          );
        }
      }
    }
  ...
  const sameValue: boolean =
    ...
    useUnchangedValue((teamData?.totalMoney ?? 0).toLocaleString(), (teamData?.baseMoney ?? 0).toLocaleString()) ||
            ...
            <TotalAssetLeft>
              ...
              {(teamData?.valProfit ?? 0) !== 0 ? (
                <ProfitContainer profit={teamData?.valProfit ?? 0}>
                  ...
                  {formattedData.valueProfit.toLocaleString()}원 ({formattedProfitNum.includes("-") ? "" : "+"}

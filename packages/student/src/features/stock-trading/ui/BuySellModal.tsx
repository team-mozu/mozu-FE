...
import { type ItemType, useGetHoldItems } from "@/entities/portfolio";
import { useGetStockDetail } from "@/entities/stock";
import { type TeamEndData, useGetTeamDetail } from "@/entities/user";
...
export const BuySellModal = ({ modalType, onClose, isOpen }: IPropsType) => {
  ...
  const [tradeData, setTradeData] = useLocalStorage<TeamEndData[]>("trade", []);
  ...
  const maxQuantity = useMemo(() => {
    ...
    if (modalType === "매수") {
      ...
      const holding = holdItemData.find((item: ItemType) => item.itemId === stockData.itemId);
  ...
  const handleConfirm = async () => {
    ...
    const existingOppositeOrder = tradeData.find(
      (tradeItem: TeamEndData) =>
        tradeItem.itemId === itemIdNum && tradeItem.orderType !== (modalType === "매수" ? "BUY" : "SELL"),
    );
    ...
    if (modalType === "매도") {
      const holding = holdItemData.find((item: ItemType) => item.itemId === itemIdNum);
    ...
    try {
      const existingIndex = tradeData.findIndex(
        (tradeItem: TeamEndData) => tradeItem.itemId === itemIdNum && tradeItem.orderType === newTradeItem.orderType,
      );

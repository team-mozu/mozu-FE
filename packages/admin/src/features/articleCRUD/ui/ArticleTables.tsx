...
import { type Article, useArticle } from "@/shared/lib/contexts/ArticleContext";
import { AddArticleItemModal } from "./AddArticleItemModal";
...
interface DisplayArticle extends LessonArticle {
  checked?: boolean;
}
...
const useArticleSelection = () => {
  ...
  const toggleAll = useCallback((articleIds: string[]) => {
        ...
        <Td width="52px">
          <CheckBoxWrapper>
            <CheckBox
              ...
              onChange={() => onToggle(article.articleId)}
              id={`article-row-${article.articleId}`}
            />
          </CheckBoxWrapper>
        </Td>
      )}
...
export const ArticleTables = memo(({ data = [], isEdit, degree, isApiLoading }: ArticleTablesProps) => {
  ...
  const currentArticles = useMemo<DisplayArticle[]>(() => {
    ...
    const articleGroup = data.find(group => group.investmentRound === round);

    if (articleGroup) {
      return articleGroup.articles.map(article => ({
        ...article,
        checked: checkedArticleIds.includes(article.articleId),
      }));
    }
    return [];
  }, [
  ...
  const handleToggleAll = useCallback(() => {
    toggleAll(currentArticles.map(article => article.articleId));
  }, [
    toggleAll,
    currentArticles,
  ]);
  ...
  const handleArticlesSelected = useCallback(
    (selectedArticles: LessonArticle[]) => {
